...
#%% Training, computing validation loss, and saving model
    train_eval.model.load_state_dict(torch.load("./models/CMU/act_sampling/layers_{0}_hid_{1}-cmu-convolutional-model-nomask-ablation.pt".format(ENC_LAYERS, HIDDEN_DIM)))
...
#%% Generating samples from validation data auto-regressively
    ...
    output_dir = os.path.join("./outputs/VESUS/convolutional/nomask_ablation", EMO_PAIR, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type = str, help = "model name", default = "vesus-convolutional-model-nomask-ablation.pt")
